...
- `install.sh`：一行安装脚本（README 中的安装方式），先检测 OpenWrt 版本（要求 25.12+，SNAPSHOT 跳过检查），再检测依赖命令（curl/sqlite3/jsonfilter/sha256sum/nslookup/ping）并用 apk 自动安装缺失的包（sqlite3 对应 sqlite3-cli 包），最后从 Release 下载、校验 SHA-256 并安装固定命名的 ipk 资产；仓库地址为 `lnwu/network-monitoring`。
